...
## Implementación con un clic de MCP Gateway

Primero, configure las variables de entorno necesarias:
...
2. Agregar un nuevo servidor MCP:
   ...
   ![Ejemplo de agregar servidor MCP](/img/add_mcp_server.png)

## Puntos de conexión disponibles

Una vez configurado, los servicios estarán disponibles en los siguientes puntos de conexión:

- MCP SSE: http://localhost:5235/mcp/user/sse
- MCP SSE Message: http://localhost:5235/mcp/user/message
- MCP Streamable HTTP: http://localhost:5235/mcp/user/mcp

Configure el cliente MCP con una URL que termine en `/sse` o `/mcp` para comenzar a usar el servicio.

## Pruebas

Puede probar el servicio de dos maneras:

1. Usar la página MCP Chat en la interfaz web
2. Usar su propio cliente MCP (**recomendado**)

## Configuración avanzada (Opcional)

Si necesita un control más granular de la configuración, puede iniciar el servicio montando archivos de configuración:
